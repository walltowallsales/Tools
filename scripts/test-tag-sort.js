...
async function main(){
 ...
 const sourceServer=await listen(source),port=await freePort(),child=spawn(process.execPath,['server.js'],{cwd:path.join(__dirname,'..','modules','tag-sort'),env:{...process.env,PORT:String(port),DATA_DIR:dataDir,SELLERCHAMP_TOKEN:'test',SELLERCHAMP_BASE_URL:`http://127.0.0.1:${sourceServer.address().port}`},stdio:['ignore','pipe','pipe']});
