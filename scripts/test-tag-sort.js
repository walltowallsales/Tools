...
async function main(){
 ...
 try{const base=`http://127.0.0.1:${port}`;let status,partialTagsChecked=false;
  for(let i=0;i<100;i++){try{const response=await fetch(`${base}/api/status`);status=await response.json();if(response.ok&&status.building&&status.progress.phase==='products'&&status.progress.products>=100&&!partialTagsChecked){const partialResponse=await fetch(`${base}/api/tags`),partial=await partialResponse.json();assert.ok(partial.tag_options.some(x=>x.tag==='auction'),'live tag options were not exposed during indexing');const searchResponse=await fetch(`${base}/api/search?tag=auction&source=product`),searchBody=await searchResponse.json();assert.equal(searchBody.count,1,'live tag search did not use the in-progress index');partialTagsChecked=true}if(response.ok&&!status.building&&status.products===100&&status.batches===1)break}catch{}await new Promise(r=>setTimeout(r,50))}
  assert.equal(status.products,100);assert.equal(status.batches,1);assert.ok(partialTagsChecked,'test did not observe the in-progress Product index');assert.ok(status.progress.rate_limited>=1,'429 retry was not exercised');
