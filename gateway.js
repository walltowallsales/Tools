...
const modules = [
  { slug: 'move', title: 'Item - Move or Update Qty', folder: 'location', port: 3101, version: 'v2.40' },
  ...
  { slug: 'tags', title: 'Item - Sort Tags by Location', folder: 'tag-sort', port: 3106, version: 'v1.10' },
  { slug: 'shipping', title: 'Shipping - Pick List', folder: 'pick', port: 3104, version: 'v31', data: 'pick' },
...
function startModule(mod) {
  const env = {
    ...
    // Pick List keeps its original server-side PIN guard. The gateway
...
app.get('/health', (req, res) => res.json({ ok: true, suite: '1.24.0', modules: modules.map(m => m.slug) }));
...
app.get('/api/dashboard/freight', async (req,res)=>{
  try{
    const r=await fetch('http://127.0.0.1:3104/api/freight',{headers:SUITE_PIN?{'x-app-pin':SUITE_PIN}:{}});
    const body=await r.json();
    if(!r.ok)return res.status(r.status).json(body);
    res.json({outstandingCount:Number(body.outstandingCount||0),overdueCount:Number(body.overdueCount||0)});
  }catch(error){res.status(503).json({error:'Pick module is still starting.',details:error.message})}
});
...
app.get('/', (req, res) => {
  const cards = modules.map(m => `<a class="card" data-module="${m.slug}" href="/${m.slug}/"><span>${m.title}</span><span class="cardMeta"><strong class="freightHomeBadge" hidden></strong><small>${m.version}</small></span></a>`).join('');
  ...
  *{box-sizing:border-box}body{margin:0;background:#f3f6fa;color:#12233b;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}header{background:#10233f;color:#fff;padding:max(26px,env(safe-area-inset-top)) 20px 24px}header div{max-width:760px;margin:auto}h1{margin:0;font-size:clamp(30px,8vw,44px)}p{margin:7px 0 0;opacity:.78}.shell{max-width:760px;margin:0 auto;padding:20px}.grid{display:grid;gap:14px}.card{display:flex;align-items:center;justify-content:space-between;min-height:84px;padding:18px 20px;border:1px solid #d7e0ec;border-radius:16px;background:#fff;color:#12233b;text-decoration:none;box-shadow:0 3px 12px #19395d12;font-size:19px;font-weight:800}.card:active{transform:scale(.99);background:#edf5ff}.cardMeta{display:flex;align-items:center;gap:8px}.card small{color:#60738a;font-size:12px;font-weight:700}.freightHomeBadge{padding:5px 9px;border-radius:999px;background:#d99100;color:#fff;font-size:11px;white-space:nowrap}.freightHomeBadge.overdue{background:#d92d20}.status{margin-top:20px;text-align:center;color:#617086;font-size:13px}.dot{display:inline-block;width:9px;height:9px;border-radius:50%;background:#22a35a;margin-right:6px}</style></head><body><header><div><h1>SellerChamp Tools</h1><p>Stuff2Sell warehouse suite · Suite v1.24</p></div></header><main class="shell"><div class="grid">${cards}</div><div class="status"><span class="dot"></span>One deployment · six isolated modules</div></main><script>fetch('/api/dashboard/freight').then(r=>r.ok?r.json():null).then(x=>{if(!x||!x.outstandingCount)return;const b=document.querySelector('[data-module="shipping"] .freightHomeBadge');b.hidden=false;b.textContent=x.outstandingCount+' freight';b.classList.toggle('overdue',x.overdueCount>0)}).catch(()=>{})</script></body></html>`);
});
